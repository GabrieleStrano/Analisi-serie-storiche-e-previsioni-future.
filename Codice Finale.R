...
data <- read_csv("C:/Users/Gabriele/Downloads/meteo/con_data.csv", 
...
par(mfrow=c(1,1), cex.axis=1.5, cex.lab=3, cex.main=3, fg="white", bg="black",
    col.axis="white", col.lab="white", col.main="white")
plot(y2, col="yellow", type= "l", main = 'Serie storica settimanale', xlab = 'Anno', ylab = 'Vendite')


#### grafico con autoplot [SERVE?]
autoplot(y2, size = 0.8, colour = 'blue') +
  ggtitle('Titolo') +
  labs(x='x', y='y') +
...
y_ts_decomp<-mstl(u1ts, s.window = "periodic")
...
#stimiamo il modello NUMERO 2

est <- Arima(smpl1, order = c(0,1,2), seasonal = list(order=c(1,0,0),period = 52))
...
autoplot(frc, ts.colour = 'black', size = 0.5) +
  ...
  ggtitle('Previsione serie storica aggregato settimanale') +
  labs(x='Anni', y='Vendite') +
  theme(legend.position = c(0.15, 0.85), legend.text=element_text(size=rel(1)), legend.title=element_text(size=rel(1.5)),
        axis.title = element_text(size = rel(1.5)), plot.title = element_text(size = rel(2)))
...
#buoni indicatori ma nel grafico alcuni picchi escono dall'intervallo di confidenza



######################################################################################

#proviamo il modello tbats a doppia stagionalità sui dati giornalieri
...
#senza stagionalità, ha senso? lo scopriremo subito
